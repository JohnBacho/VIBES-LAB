...
public class ElevatorDoorController : MonoBehaviour
{
    ...
    {
        while (Vector3.Distance(leftDoor.transform.localPosition, leftDoorClosedPos) > 0.01f)
        {
            leftDoor.transform.localPosition = Vector3.Lerp(leftDoor.transform.localPosition, leftDoorClosedPos, Time.deltaTime * openSpeed);
            rightDoor.transform.localPosition = Vector3.Lerp(rightDoor.transform.localPosition, rightDoorClosedPos, Time.deltaTime * openSpeed);
            yield return null;
        }
