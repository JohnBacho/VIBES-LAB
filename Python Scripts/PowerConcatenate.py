...
df2 = pd.read_csv(EyeTrackerPath)
df3 = pd.read_csv(AnticipatePath)

merge_df = df2.merge(
    df3[["SubjectID", "Date", "LocalTime", "UnityTime", "Phase", "BlockNumber", "TrialNumber", "Step", "TrialTime", "Anticipated", "ResponseTime"]],
    on=["SubjectID", "Date", "LocalTime", "UnityTime", "Phase", "BlockNumber", "TrialNumber", "Step", "TrialTime"],
    how="left"
)
...
filename_full = f"{output_dir}\\Subject{int(subject_id)}Date{str(date)}.csv"
